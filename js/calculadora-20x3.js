import {
  ...
  LISTA_MENSUAL_20X3_CLIENTE_NUEVO,
  LISTA_MENSUAL_20X3_CLIENTE_BASE,
  LISTA_20x3,
  LISTA_LINEAS_20x3,
  B2_C12_RANGO,
  B14_C24_RANGO,
...
const HELPERS = {
  ...
  establecer_datos() {
    ...
    const pago_ahorro_anual_familiar = (pago_sin_descuento * 12) - ((pago_con_descuento * 9) + (pago_con_20x3 * 3));
  ...
  pago_20x3(q_lineas, tipo_cliente, pago_mensual_con_familia, plan_familiar, lineas_portar, total_lineas) {
    ...
    if (tipo_cliente === "NUEVO") {
      return LISTA_MENSUAL_20X3_CLIENTE_NUEVO.find(e => e.plan === plan_familiar && e.linea === total_lineas)?.monto;  // L73
    }

    return LISTA_MENSUAL_20X3_CLIENTE_BASE.find(e => e.linea_base === q_lineas && e.linea === total_lineas && e.plan === plan_familiar)?.monto;
  }
};
